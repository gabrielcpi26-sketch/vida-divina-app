    ...
    <section id="payments" className="px-4 md:px-6 py-8">
      ...
      <p className="text-sm text-gray-600 mb-6">
        {isAdmin
          ...
          : "Consulta tus formas de pago disponibles. Los datos se muestran solo para lectura."}
      </p>
        ...
        <div className="p-4 border rounded-2xl bg-white shadow-sm">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              onClick={() => copy(data.paypal.email)}
            >
              Copiar email
            </button>
          </div>
          ...
          <input
            ...
            onChange={(e) => onChange("paypal", "email", e.target.value)}
          ...
          <input
            className="w-full border rounded px-2 py-1 mb-2 text-sm"
            value={data.paypal.note}
            onChange={(e) => onChange("paypal", "note", e.target.value)}
          ...
          {isAdmin && (
            <>
              ...
              <input
                ...
                disabled={!isAdmin}
                onChange={(e) => onFile("paypal", e.target.files?.[0])}
              />
            </>
          )}
          ...
          {data.paypal.qr ? (
            <img
              src={data.paypal.qr}
              alt="QR PayPal"
              className="w-full h-36 object-contain border rounded"
            />
          ) : (
            <div className="h-36 flex items-center justify-center border rounded text-xs text-gray-400">
              No hay QR
            </div>
          )}
        </div>
        ...
        <div className="p-4 border rounded-2xl bg-white shadow-sm">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-2">
              <button
                className="text-xs px-2 py-1 border rounded"
                onClick={() => copy(data.bbva.clabe)}
              >
                Copiar CLABE
              </button>
              <button
                className="text-xs px-2 py-1 border rounded"
                onClick={() => copy(data.bbva.account)}
              >
                Copiar cuenta
              </button>
            </div>
          ...
          <input
            className="w-full border rounded px-2 py-1 mb-2 text-sm"
            value={data.bbva.clabe}
            onChange={(e) => onChange("bbva", "clabe", e.target.value)}
          ...
          <input
            className="w-full border rounded px-2 py-1 mb-2 text-sm"
            value={data.bbva.account}
            onChange={(e) => onChange("bbva", "account", e.target.value)}
          ...
          <input
            className="w-full border rounded px-2 py-1 mb-2 text-sm"
            value={data.bbva.holder}
            onChange={(e) => onChange("bbva", "holder", e.target.value)}
          ...
          {isAdmin && (
            <>
              ...
              <input
                ...
                className="w-full mb-2 text-sm"
                disabled={!isAdmin}
                onChange={(e) => onFile("bbva", e.target.files?.[0])}
              />
            </>
          )}
        ...
        <div className="p-4 border rounded-2xl bg-white shadow-sm">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-2">
              <button
                className="text-xs px-2 py-1 border rounded"
                onClick={() => copy(data.banorte.clabe)}
              >
                Copiar CLABE
              </button>
              <button
                className="text-xs px-2 py-1 border rounded"
                onClick={() => copy(data.banorte.account)}
              >
                Copiar cuenta
              </button>
            </div>
          ...
          <input
            className="w-full border rounded px-2 py-1 mb-2 text-sm"
            value={data.banorte.clabe}
            onChange={(e) => onChange("banorte", "clabe", e.target.value)}
            placeholder="012345678901234567"
            readOnly={!isAdmin}
            disabled={!isAdmin}
          />
          ...
          <input
            className="w-full border rounded px-2 py-1 mb-2 text-sm"
            value={data.banorte.account}
            onChange={(e) => onChange("banorte", "account", e.target.value)}
            placeholder="1234567890"
            readOnly={!isAdmin}
            disabled={!isAdmin}
          />
          ...
          <input
            className="w-full border rounded px-2 py-1 mb-2 text-sm"
            value={data.banorte.holder}
            onChange={(e) => onChange("banorte", "holder", e.target.value)}
            placeholder="Nombre del titular"
            readOnly={!isAdmin}
            disabled={!isAdmin}
          />
          ...
          {isAdmin && (
            <>
              ...
              <input
                ...
                className="w-full mb-2 text-sm"
                disabled={!isAdmin}
                onChange={(e) => onFile("banorte", e.target.files?.[0])}
              />
            </>
          )}
        ...
        {isAdmin && (
          <>
            <button
              ...
              onClick={() => alert("Datos guardados localmente")}
            >
              Guardar
            </button>
            <button className="px-4 py-2 rounded border" onClick={resetAll}>
              Restablecer
            </button>
            <button className="px-4 py-2 rounded border" onClick={exportJSON}>
              Exportar (JSON)
            </button>
          </>
        )}
